...
public class MovieRepository : IMovieRepository
{
    ...
    // Maps caller-supplied sort fields to trusted column names so the value is never
    // interpolated into SQL directly, even if request validation is bypassed.
    ...
    public async Task<IEnumerable<Movie>> GetAllAsync(GetAllMoviesOptions options, CancellationToken token = default)
    {
        ...
        if (options.SortField is not null
            && SortableColumns.TryGetValue(options.SortField, out var sortColumn))
        {
            orderClause = $"""
            , m.{sortColumn}
